...
### Decision taken 2026-09-22

| # | Question | Answer |
|---|---|---|
| Q51 | The capacity-policy API becomes **beta at public preview**, needing `?beta=true` on every call, and drops it again at GA. How is that carried? | **A `Two options` environment variable, `ubsppcoe_PolicyApiBeta`**, read by the six Fabric calls in four flows, each URL ending `@{if(parameters('PolicyApiBeta (ubsppcoe_PolicyApiBeta)'), '?beta=true', '')}`. Both transitions are then a toggle, with no flow reopened. **A Text variable was tried first and the platform rejected every form of it**: an empty value cannot be stored — the run fails with *value was not found* and dependent flows will not publish; clearing a value is silently ignored, so the flow keeps the previous one; and a bare `?` is rejected by Fabric with `400`. Confirmed 2026-09-22 against `Activate`. Full record in [API-BETA-SWITCH.md](API-BETA-SWITCH.md) |
