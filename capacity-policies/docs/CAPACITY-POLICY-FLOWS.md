...
8. Activate — `POST /v1/workspaces/{holderWs}/policySets/{id}/activate`, body `{ scopeType: "Capacity", scopeId: capacityId, capacityId: capacityId }`. **`capacityId` is undocumented but required** — without it the preview service returns `PropertyCannotBeDefault`; see `activate_policy_set.ps1`. **Treat `PolicySetAlreadyActive` as success** — it arrives as a `400` with that `errorCode`, and the end state is the one wanted. `PolicySetActivationConflict` means another set already owns the capacity and needs `allowReplace` — a **query parameter**, `?allowReplace=True`, not a body property. Do **not** pass it blindly; surface it and let a human decide.
...
| Report recipients | Environment variable `ubsppcoe_PolicyMailRecipients` — Text | A **semicolon-separated** address list, read by the `To` field of all four report emails in the three `MIG_` flows. One distribution group per environment is preferred to a list of individuals — [CAPACITY-POLICY-TABLES.md](CAPACITY-POLICY-TABLES.md) §8.11 |
...
The two limits are environment variables so that a service-side change does not need a flow edit. **`PolicyApiBeta` exists for the same reason** — the API's move to beta at public preview, and back at GA, changes only the URL, so a toggle carries both transitions. **`PolicyMailRecipients` exists for the third** — who gets the migration reports is per environment and changes with the team, not with the design.
