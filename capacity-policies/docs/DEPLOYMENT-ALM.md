...
Seven, created inside the solution so they inherit the `ubsppcoe_` prefix. **All Text except `PolicyApiBeta`**, which is Two options.
...
| `PolicyMailRecipients` | `ubsppcoe_PolicyMailRecipients` | **Per environment** — a semicolon-separated address list for the `MIG_` flow reports |
...
**There is no policy-name variable.** `ItemCreation` is a literal in the rebuild body. Do not create an eighth variable unless you also parameterise that body.
...
> **`PolicyMailRecipients` is the one variable that should differ between environments on purpose.** Dev and test point at a test mailbox; only production points at the real distribution group. That is what stops a dev-environment migration run from mailing the platform team.
>
> **It is Text, so it must never be blank and must never be cleared** — a blank value stops the three `MIG_` flows publishing, and a cleared one is silently ignored while the old list keeps receiving mail. **Put a generic mailbox in the Default Value and keep named individuals out of it**, because the default is what travels with the export and lands in source control. Full guidance in [CAPACITY-POLICY-TABLES.md](CAPACITY-POLICY-TABLES.md) §8.11.
